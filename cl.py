...
class ContrastiveDataset(Dataset):
    def __init__(self, question_file, i2c_path, model, tokenizer, conv_mode):
        self.model = model
    ...
    def __getitem__(self, idx):
        ...
        for i in range(0, len(conversations), 2):
            ...
            input_ids = self.tokenizer(qa, return_tensors="pt").input_ids
...
def contrastive_loss(anchor, positive, negatives, temperature=0.2):
    ...
    if torch.isnan(loss).any() or torch.isinf(loss).any():
        raise ValueError("NaN or Inf detected in loss calculation")
...
def contrastive_learning_train(args, device="cuda"):
    ...
    for epoch in range(args.epochs):
        for batch_idx, (descriptions, positive, negatives) in enumerate(dataloader):
            ...
            # Move input tensors to device (no-op if already on device)
            input_ids = tokenizer(
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                print(f"NaN or Inf loss detected at batch {batch_idx}, skipping update")
                continue
